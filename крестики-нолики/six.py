...
pygame.display.set_caption("Крестики-нолики 6x6")
...
def check_winner(board, player):
    ...
    for i in range(6):
        ...
        for j in range(3):
            if all(board[j + k][i] == player for k in range(4)):
                return True
    ...
    for i in range(3):
        for j in range(3):
            ...
            if all(board[i + k][5 - j - k] == player for k in range(4)):
                return True
...
def check_draw(board):
    return all(board[i][j] != ' ' for i in range(6) for j in range(6))
...
def draw_score():
    ...
    text = font1.render(f"Для победы необходимо набрать 4 элемента в ряд", True, neon_white)
