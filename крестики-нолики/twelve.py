...
pygame.display.set_caption("Крестики-нолики 12x12")
...
def draw_symbol(row, col, symbol):
    ...
    if symbol == 'X':
        pygame.draw.line(screen, neon_green, (x - 20, y - 20), (x + 20, y + 20), 3)
        pygame.draw.line(screen, neon_green, (x + 20, y - 20), (x - 20, y + 20), 3)
    elif symbol == 'O':
        pygame.draw.circle(screen, neon_yellow, (x, y), 20, 3)
...
def check_winner(board, player):
    ...
    for i in range(12):
        # Проверка по строкам
        for j in range(7):
            if all(board[i][j + k] == player for k in range(6)):
                return True

        # Проверка по столбцам
        for j in range(7):
            if all(board[j + k][i] == player for k in range(6)):
                return True
    ...
    for i in range(7):
        for j in range(7):
            ...
            if all(board[i + k][j + k] == player for k in range(6)):
                return True

            # Проверка по диагоналям справа налево (/)
            if all(board[i + k][11 - j - k] == player for k in range(6)):
                return True
...
def check_draw(board):
    return all(board[i][j] != ' ' for i in range(12) for j in range(12))
...
def draw_score():
    ...
    text = font1.render(f"Для победы необходимо набрать 8 элемента в ряд", True, neon_white)
